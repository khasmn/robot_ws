...
class SerialBridge(Node):
    def __init__(self):
        ...
        self.get_logger().info("SerialBridge initialized. Waiting for homing to complete...")
            ...
            if self.ser.in_waiting > 0:
                ...
                if line:
                    ...
                    if "HOMING_COMPLETE" in line:
                        # Run gripper self-test in a separate thread so we
                        # don't block the serial reader loop
                        threading.Thread(
                            target=self._gripper_self_test, daemon=True
                        ).start()

    # ------------------------------------------------------------------
    # Gripper self-test — runs once immediately after homing completes
    #
    # Sequence:
    #   1. CLOSE for 1 s
    #   2. OPEN  for 1 s
    #   3. STOP  (leave gripper open and idle — ready to grab)
    #   4. Publish /robot_ready so YOLO node can start accepting targets
    # ------------------------------------------------------------------
    def _gripper_self_test(self):
        ...
        # Step 1 — Close
        self.get_logger().info("[Self-test] Step 1/2: Closing gripper...")
        self.gripper.close(duration=1.0)
        time.sleep(1.2)          # motor duration + 0.2 s buffer

        # Step 2 — Open
        self.get_logger().info("[Self-test] Step 2/2: Opening gripper...")
        self.gripper.open(duration=1.0)
        time.sleep(1.2)

        # Step 3 — Explicit stop to make sure motor is off
        self.gripper.stop()

        self.get_logger().info("=== Gripper self-test complete. Gripper is OPEN and idle. ===")
        self._mark_ready()

    # ------------------------------------------------------------------
    # Mark robot as ready — called after self-test finishes
    # ------------------------------------------------------------------
    def _mark_ready(self):
        ...
        self.get_logger().info("=== Robot is READY. Published /robot_ready. ===")
    ...
    def gripper_command_callback(self, msg: String):
        ...
        if cmd == 'close':
            self.gripper.close(duration=1.5)
            threading.Timer(1.6, self._publish_gripper_status, args=('closed',)).start()

        elif cmd == 'open':
            self.gripper.open(duration=1.5)
            threading.Timer(1.6, self._publish_gripper_status, args=('opened',)).start()
            ...
            self.get_logger().warn(
                f"Unknown gripper command: '{cmd}'. Use 'open', 'close', or 'stop'.")
