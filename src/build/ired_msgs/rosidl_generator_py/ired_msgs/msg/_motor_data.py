...
class MotorData(metaclass=Metaclass_MotorData):
    ...
    def __init__(self, **kwargs):
        ...
        if 'speed_sp' not in kwargs:
            self.speed_sp = numpy.zeros(4, dtype=numpy.float64)
        else:
            self.speed_sp = kwargs.get('speed_sp')
        if 'speed_fb' not in kwargs:
            self.speed_fb = numpy.zeros(4, dtype=numpy.float64)
        else:
            self.speed_fb = kwargs.get('speed_fb')
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if any(self.speed_sp != other.speed_sp):
            return False
        if any(self.speed_fb != other.speed_fb):
            return False
        ...
        if self._check_fields:
            if isinstance(value, numpy.ndarray):
                assert value.dtype == numpy.float64, \
                    "The 'speed_sp' numpy.ndarray() must have the dtype of 'numpy.float64'"
                assert value.size == 4, \
                    "The 'speed_sp' numpy.ndarray() must have a size of 4"
            ...
            assert \
                ...
                "The 'speed_sp' field must be a set or sequence with length 4 and each value of type 'float' and each double in [-.000000, .000000]"
        self._speed_sp = numpy.array(value, dtype=numpy.float64)

    @builtins.property
    def speed_fb(self):
        """Message field 'speed_fb'."""
        return self._speed_fb

    @speed_fb.setter
    def speed_fb(self, value):
        if self._check_fields:
            if isinstance(value, numpy.ndarray):
                assert value.dtype == numpy.float64, \
                    "The 'speed_fb' numpy.ndarray() must have the dtype of 'numpy.float64'"
                assert value.size == 4, \
                    "The 'speed_fb' numpy.ndarray() must have a size of 4"
                self._speed_fb = value
                return
            from collections.abc import Sequence
            from collections.abc import Set
            from collections import UserList
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -1.7976931348623157e+308 or val > 1.7976931348623157e+308) or math.isinf(val) for val in value)), \
                "The 'speed_fb' field must be a set or sequence with length 4 and each value of type 'float' and each double in [-.000000, .000000]"
        self._speed_fb = numpy.array(value, dtype=numpy.float64)
