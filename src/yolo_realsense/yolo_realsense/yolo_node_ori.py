...
class YoloDistanceNode(Node):
    def __init__(self):
        ...
        # --- Configuration ---
        self.model_path = os.path.join(get_package_share_directory('yolo_realsense'), 'yolo_realsense', 'yolosegv1.pt')
                ...
                if result.boxes is not None:
                    for i, box in enumerate(result.boxes):
                        ...
                        try:
                            ...
                            Y_tr = -1.5847 * Y_tr +154.4881
                            Y_ori = pt_transformed.point.y
                            Z_tr = pt_transformed.point.z + 30
                        ...
                        if self.show_view:
                            ...
                            coord_text = f"Gripper X:{int(Y_tr)} Xo:{int(Y_ori)}  Y:{int(X_tr - 220)}  Z:{int(Z_tr + 30)}"
                            cv2.putText(
                                vis_image,
                                coord_text,
                                (x1, y2 + 20),
